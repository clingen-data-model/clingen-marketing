 ...
 * Fetch Documents
 */
$fetch = $pages->find("template=announcement-type-event-show|announcement-type-post-show|announcement-type-poster-show|announcement-type-presentation-show|document-type-publication-show|announcement-type-pubmention-show");
if($fetch) {
...
if($fetch) {
    $render_item .= "
      <a href='{$page->url}' class=\"list-group-item list-group-item-action d-flex justify-content-between align-items-center\">All Announcements<span class=\"badge badge-pill badge-secondary\">{$document_count}</span></a>
    ";
  foreach ($fetch as $key => $value) {
